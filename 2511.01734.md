...
2. Beyond the first step, the network output becomes a **polynomial in η with growing degree**, so controlling higher-order monomials and their width-limits is nontrivial.

### Key Mechanism (How they do it)

- **Set-up & definition.** They formalize “LR transfer” as: the optimal LR η*n^(t) converges (in probability) to some deterministic η*∞^(t) > 0 as width n→∞.
- **Polynomial-in-η view.** Show the t-step model output f^(t)(x) is a **polynomial in η** whose coefficients (determined by initialization) converge almost surely to deterministic limits via Tensor Programs. Hence, the t-step loss L*n^(t)(η) converges uniformly on compacts to a deterministic polynomial L*∞^(t)(η).
- **Step t = 1 in detail.** For one step, coefficients of degree ≥2 shrink with width; the degree-1 term converges to a **non-zero** constant. The loss becomes “quasi-quadratic” in η, enabling an explicit limit and a rate for the optimizer.
...
- **Theorem 1 (t=1):** The optimal LR under µP converges to a **strictly positive constant** η*∞^(1) with rate **O(n^{-1/2})** on compact η-intervals.
  \_Empirics:* On synthetic linear data, measured optima across widths {2^k} concentrate around the theoretical limit; the observed rate matches n^{-1/2} up to ~1024 width before slowing (Fig. 2, page 7).
- **Failure under SP/NTP:** Under **SP**, η_n^(1) → 0 as n→∞, so LR does **not** transfer; under **NTP**, the effective LR scales down with n^{-1/2} and the optimal LR blows up—again, no transfer.
- **General t (Theorem 3):** For any step t, f^(t)(x) remains a polynomial in η with coefficients converging almost surely; with a mild uniqueness assumption, the optimal LR converges almost surely to a **non-zero** constant, i.e., LR transfer holds at **any finite training step**.
...
- **Clean mechanism:** The polynomial-in-η lens makes the phenomenon intuitive and tractable; Tensor Programs delivers almost-sure convergence of coefficients.
...
- Viewing training **step-t** behavior as a **deterministic polynomial limit in η** is a simple, powerful abstraction for hyperparameter transfer proofs.
...
The paper proves that for deep **linear** MLPs trained with GD under **µP**, the **optimal learning rate** converges to a **non-zero constant** as width grows; at **t=1** it converges at **O(n^{-1/2})**. Under **SP**, the optimal LR → 0; under **NTP**, it diverges—so no transfer there. The core technique is to express t-step outputs as **polynomials in η** and show their coefficients **converge** almost surely via Tensor Programs, which stabilizes the argmin. Experiments back the theory, and suggest similar transfer for **ReLU + Adam**, though that extension remains open.
