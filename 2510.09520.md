...
1. **Adaptive GHZ compiler on the hardware graph**
   - Root at a vertex of near-minimal eccentricity and grow a **BFS tree** to minimize CNOT depth (D\_{\text{GHZ}} \lesssim \text{radius}(G)). (Eq. (2), p.3)
...
3. **Temporary uncomputation** to fight dephasing
   - **ASAP uncompute / ALAP recompute** early nodes so they relax in (|0\rangle) during long idle spans (plus **dynamical decoupling**). (p.3; App. B + **Fig. 3** p.8)

4. **Readout-aware fidelity estimation**
   - Run **Parity Oscillations** (coherence (\chi) via Fourier of (M\_\phi)) + direct **DFE** over GHZ stabilizers; show the two are **theoretically equivalent** and empirically agree. Use **TREX** (expectation-value) and **M3** (bitstring) mitigation appropriately. (p.4–5; App. A p.7)
