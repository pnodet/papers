...
1. **Sparsity-regularized autoencoder** maps (H*t) to latent ( \hat Z_t ) and reconstructs (H_t) while promoting a sparse decoder Jacobian (J*{\hat f}) (Eq. 6–7).
2. **Structure-aware routing**: compute per-latent **agreement** across agents (Eq. 8–10), then give each agent only the relevant ( \hat Z\_{t} ) coordinates, reweighted by agreement level.
